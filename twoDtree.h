...
public:
    ...
    */
   PNG render();
   void render(Node* subroot, PNG & image);
    ...
    */
   void prune(int tol);
   void prune(Node* currentnode, int tol);
   bool prunehelper(Node * currentnode, int tol, RGBAPixel & avg);
   ...
   int pruneSizeHelper(Node* currentnode, int tol);
...
private:
   ...
   // Node * buildTree(stats & s, pair<int,int> ul, pair<int,int> lr, bool vert, long preVar);
   // Node * buildTree(stats & s, pair<int,int> ul, pair<int,int> lr, bool vert, long preHVar, long preVVar);



   /**
    * Returns the distance between two pixels
    * Distance is defined as the sum, over each color channel,
    * of the pixel value differences, squared.
    */
   double dist(RGBAPixel p1, RGBAPixel p2);
